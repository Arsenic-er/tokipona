...
2026-09-13 已交付 F5 森林开场整合候选：石木连续运动、共享水土材料、自由过溪与原任务／存档桥接；不是完整三小时第一章。已有主档请按 J →“临时重玩（不改主存档）”体验新物理。实机录像为 `exports/windows/tokipona-rpg-review.webm`；统一验收范围、实测性能和已知问题见 [F3–F5 验收记录](docs/testing/2026-09-13-forest-f3-f5-integration.md)。

开发交付后运行 `pnpm run desktop:latest`：先构建与验证新 EXE 的启动、输入和跨启动读档，成功才替换同名文件，清理构建临时目录，不留旧版游戏副本。失败时保留现有最新版；替换前请关闭游戏。不得把存档目录当作旧版本清理目标。
